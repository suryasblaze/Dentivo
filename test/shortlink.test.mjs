...
ok('short enough to read out', link.length < 55, `${link.length} chars — ${link}`)
ok('the id is unguessable', /\/b\/[0-9a-z]{14}$/.test(link), link)
...
console.log('\n4. Ids do not repeat')
const ids = new Set()
for (let i = 0; i < 200; i++) ids.add((await billLink({ clinic, patient, visit: { ...visit, id: 'x' + i }, bill })).split('/b/')[1])
ok('200 links, 200 different ids', ids.size === 200, ids.size)
ok('every id is 14 characters', [...ids].every(x => /^[0-9a-z]{14}$/.test(x)))
